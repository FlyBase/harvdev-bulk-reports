...
# report_current_gene_product_ids.py
# author: Gil dos Santos
# usage: report_current_gene_product_ids.py [-h] [-v VERBOSE]
################################################################################
...
def main():
    ...
    to_export_as_tsv['metaData']['title'] = 'FlyBase current_gene_product report'
    ...
    SELECT DISTINCT g.uniquename, t.uniquename, p.uniquename
    FROM feature g
    ...
    fb_ncgene_query = """
    SELECT DISTINCT g.uniquename, t.uniquename
    FROM feature g
    JOIN feature_relationship tg ON (tg.object_id = g.feature_id)
    JOIN feature t ON (t.feature_id = tg.subject_id)
    JOIN featureloc flt ON (flt.feature_id = t.feature_id)
    ...
    fb_mirna_query = """
    SELECT DISTINCT g.uniquename, mi.uniquename
    FROM feature g
    JOIN feature_relationship tg ON (tg.object_id = g.feature_id)
    JOIN feature t ON (t.feature_id = tg.subject_id)
    JOIN featureloc flt ON (flt.feature_id = t.feature_id)
    ...
    for line in gene_product_info:
        this_dict = {}
        this_dict['FlyBase_FBgn'] = line[0]
        this_dict['FlyBase_FBtr'] = line[1]
        if len(line) == 3:
            this_dict['FlyBase_FBpp'] = line[2]
        else:
            this_dict['FlyBase_FBpp'] = None
