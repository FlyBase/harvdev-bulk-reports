...
print "##insertion_symbol\tFBti#\tgenomic_location\trange\torientation\testimated_cytogenetic_location\tobserved_cytogenetic_location\n";
...
my $fbtiwc = 'FBti%';
## Main driver
my $iq = $dbh->prepare(sprintf("SELECT i.uniquename, i.name, i.feature_id, cvt.name as ftype from feature i, cvterm cvt where i.type_id = cvt.cvterm_id and i.is_obsolete = 'f' and is_analysis = 'f' and cvt.name = 'transposable_element_insertion_site' and uniquename like '%s'",$fbtiwc));
## Test driver (Limited)
...
## Get genomic location (if any), orientation and range 
    ...
    my $orien;
    my $range;
    my $gq =  $dbh2->prepare(sprintf("SELECT fmin, fmax, strand, is_fmin_partial, is_fmax_partial, g.uniquename as arm from featureloc fl, feature g  where fl.feature_id = %d and fl.srcfeature_id = g.feature_id",$ir{feature_id}));
...
## estimated_cytogenetic_location, observed_cytogenetic_location)
    print(sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s\n",$ir{name},$ir{uniquename},$gloc,$range,$orien,$est_cloc,$cur_cloc));
}
