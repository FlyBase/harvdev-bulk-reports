...
$header .= "!date: $DATE \$\n!from: FlyBase\n!saved-by: FlyBase GOcur gocur\@morgan.harvard.edu\n!Data from FlyBase version $RELEASE\n";
  ...
  (sprintf
   ("SELECT fcvtp.feature_cvterm_id, qual.name
     ...
     WHERE  fcvtp.type_id = qual.cvterm_id and
            qual.name IN ('contributes_to', 'colocalizes_with')"
   )
  );
...
#      print STDERR "NO PMID for $pub\n";
  ...
  # handle negation and qualifiers (col 4).
  if ($is_not and exists($quals{$fcvtid})) {
      $line .= "NOT|$quals{$fcvtid}\t";
  } elsif (exists($quals{$fcvtid})) {
      $line .= "$quals{$fcvtid}\t";
  } elsif ($is_not) {
      $line .= "NOT\t";
  } else {
