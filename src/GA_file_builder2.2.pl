  ...
  )
{
    ...
    # cols 5 and 6
    # $line .= "GO:$goid\tFB:$pub";    # DB-1002: Suppress FBrf ID from the pub column. Leaving old code in case reversion requested.
    $line .= "GO:$goid\t";
    # print_log("DEBUG: 6. Filled in cols 5-6: GO & FB pub IDs.");

    # check for PMID
    # $line .= "|$pmid" if $pmid;    # DB-1002: Remove pipe since preceding FBrf ID is now suppressed.
    $line .= "$pmid" if $pmid;
    $line .= "\t";
